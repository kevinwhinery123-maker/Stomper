...
function buildPrescription(profile, sessions, signals) {
  ...
  const prescriptions = {
    run_stronger: { label: 'Improve running', headline: signals.suggestedMiles ? `${signals.suggestedMiles} mile running target` : 'Build a repeatable running base', metrics: [{ value: signals.suggestedMiles ? `${signals.suggestedMiles} mi` : `${aerobicDays} runs`, label: 'weekly target' }, { value: '70%+', label: 'easy running' }, { value: String(Math.min(hardDays, 1)), label: 'quality session' }, { value: String(strengthDays), label: 'support session' }], guidance: 'Most running stays conversational. Tempo increases one stressor at a time and holds progression after hard or inconsistent weeks.', ...shared },
    build_strength: { label: 'Build muscle + strength', headline: `${strengthDays} focused lifting days`, metrics: [{ value: String(strengthDays), label: 'lifting days' }, { value: '~10', label: 'sets per muscle goal' }, { value: '2–4', label: 'sets per exercise' }, { value: '1–3', label: 'reps in reserve' }], guidance: 'Tempo uses repeatable movement patterns and double progression: reach the top of a rep range with good form, then add a small amount of load.', ...shared },
    ...
    both: { label: 'Hybrid performance', headline: 'Running and lifting without stacking hard days', metrics: [{ value: String(aerobicDays), label: 'running days' }, { value: String(strengthDays), label: 'lifting days' }, { value: String(hardDays), label: 'demanding days' }, { value: String(scheduledMinutes), label: 'planned minutes' }], guidance: 'Tempo balances running and lifting, separates demanding sessions when possible, and protects consistency before adding volume.', ...shared }
  };
