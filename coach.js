...
function previewReply(plan, message, actionSummary = '') {
  ...
  if (/progress|improv|review|week|summary|data/i.test(message)) { const fitness=plan.fitnessSummary, recent=fitness?.windows?.current7, dataRead=recent?`Your last 7 days include ${recent.completedSessions} completed session${recent.completedSessions===1?'':'s'}, ${recent.trainingMinutes} training minutes${recent.runningMiles?`, ${recent.runningMiles} running miles`:''}${recent.liftingSets?`, and ${recent.liftingSets} lifting sets`:''}. ${fitness.coachingSignals[0]}`:`This week, you have completed ${plan.summary.completed} of ${plan.summary.planned} planned sessions.`;return `${dataRead} ${plan.adjustment || 'Keep logging what you actually do so Tempo can make the next recommendation more specific.'} Next focus: build the next win from consistency, not a big jump.`;}
